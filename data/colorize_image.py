...
class ColorizeImageBase():
    ...
    # def set_img_lab(self,input_img_lab):
    #     # INPUT
    #     #     input_img_lab     CxXdxXd
    #     self.img_lab_fullres = input_img_lab.copy()
    #     self.img_lab = input_img_lab.copy()
...
class ColorizeImageCaffe(ColorizeImageBase):
    ...
    def prep_net(self,gpu_id,prototxt_path='',caffemodel_path=''):
        ...
        if(len(self.net.params[self.pred_ab_layer][0].data[...].shape)==4 \
    ...
    # img_l = cid.img_lab[[0],:,:].transpose((1,2,0))
    def get_img_forward(self):
        # get image with point estimate
        # return lab2rgb_transpose(self.img_l,self.net.blobs[self.pred_ab_layer].data[0,:,:,:])
        return self.output_rgb
...
class ColorizeImageTorch(ColorizeImageBase):
    def __init__(self,Xd=256):
        ...
        self.torch = lua.require("torch")
        lua.require("image")
        lua.require("nn")
        self.util = lua.require("util.util")

    def prep_net(self,net_path,gpu_id):
        print('gpu_id = %d, net_path = %s' % (gpu_id, net_path))
        self.net = self.util.load_gpu_id(net_path,gpu_id)
        self.gpu_id = gpu_id
        self.net_set = True
        if self.gpu_id:
            lua.require("cunn")
            self.util.cudnn(self.net)
            self.net._cuda()
    ...
    def net_forward(self,input_ab,input_mask):
        ...
        net_input_prepped = np.concatenate((self.img_l_mc,self.input_ab_mc,self.input_mask_mult),axis=0)
        net_input = self.torch.fromNumpyArray(net_input_prepped[np.newaxis,:,:,:])
        if self.gpu_id:
            net_input = net_input._cuda()

        # self.out_ab_mc = self.torch.toNumpyType(self.net._forward(net_input))
        a = self.util.deprocessLAB_batch(\
            self.torch.fromNumpyArray(self.img_l_mc[np.newaxis,:,:,:]), \
            self.net._forward(net_input))
        return np.clip(a[0].asNumpyArray().transpose((1,2,0))*255,0,255).astype('uint8')

    def get_img_forward(self):
        # Get the image you called forward on (L concatenated with input ab values)
        a = self.util.deprocessLAB_batch(\
            self.torch.fromNumpyArray(self.img_l_mc[np.newaxis,:,:,:]), \
            self.torch.fromNumpyArray(self.input_ab_mc[np.newaxis,:,:,:]))
        return np.clip(a[0].asNumpyArray().transpose((1,2,0))*255,0,255).astype('uint8')
    ...
    def get_img_gray(self):
        ...
        a = self.util.deprocessLAB_batch(\
            self.torch.fromNumpyArray(self.img_l_mc[np.newaxis,:,:,:]), \
            self.torch.fromNumpyArray(np.zeros((1,2,self.Xd,self.Xd))))
        return np.clip(a[0].asNumpyArray().transpose((1,2,0))*255,0,255).astype('uint8')



    def _set_img_lab_(self):
        ColorizeImageBase._set_img_lab_(self)
        plt.imsave(os.path.join(self.image_dir,'cur_img.png'),self.get_img_gray())
        # plt.imsave(os.path.join(self.image_dir,'cur_img.png'),self.img_rgb)
