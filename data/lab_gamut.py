...
def qcolor2lab_1d(qc):
	...
	c = np.array([qc.red(), qc.green(), qc.blue()], np.uint8)#astype('uint8')
	# print('qcolor', c)
	return rgb2lab_1d(c)
...
class abGrid():
	def __init__(self, gamut_size=110, D=1):
		self.D = D
		self.vals_b, self.vals_a = np.meshgrid(np.arange(-gamut_size,gamut_size+D,D),np.arange(-gamut_size, gamut_size+D,D))
